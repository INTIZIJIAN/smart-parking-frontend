...
export default function Home() {
  ...
  <span className="ml-3 text-sm font-medium text-white">Playground</span>
</label>
      </div>
  ...
  <div className="flex flex-wrap justify-center gap-6">
    {Object.entries(
      availableSpot.reduce((acc: Record<string, ParkingSpot[]>, spot: ParkingSpot) => {
      ...
      <div key={zone} className="bg-white p-4 rounded-lg shadow-md w-auto">
        ...
        <ul className="grid grid-cols-2 gap-3 text-black">
          {spots.map((spot: ParkingSpot) => (
            ...
            >
              ...
              <div>{spot.available ? "✅ Available" : "❌ Unavailable"}</div>
            </li>
          ))}
        </ul>
      </div>
